async function getProjects() {
    ...
    // Here we use the `await` keyword to signal to Javascript that itshouldn't run this code until `response` gets turned into JSON
        ...
        // If the error response *is* JSON, then we will include the info fromthat JSON in the error we throw.// Usually, the server will send the error message in the `detail`property.
